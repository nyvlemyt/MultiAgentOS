...
3. ~~Backfill the worst gap first: `seed.ts` (or carve its logic into testable units), then `client.ts` / `testing.ts`.~~ **DONE 2026-06-26** (PR #49). `seed.ts` was settled by *exclusion* (dev-only seeding script, vitest.config exclude) rather than tests, so the live worst-first real-logic gaps were backfilled instead: `db/src/client.ts` 67.6 % → **100 %** (new `client.test.ts` — `findRepoRoot` made load-bearing via outer-marker / marker-less-subdir fixtures), `core/src/llm.real.ts` 83.8 % → **98.8 %** (scriptable SDK mock drives the rate_limit/generic failure path), `agents/src/sandbox-diff.ts` 83.6 % L · 50 % br → **96.7 %** (git-init-on-non-repo + outer catch; residual 45,80 are defensive `unlinkSync` catch-ignores), `core/src/providers/openai-compat.ts` 25 % br → **100 %**. No real-logic file remains below the §7 80 % line bar — the next-lowest (`db/src/seed-safety.ts` 83.3 %, `core/src/providers/factory.ts` 86.8 %) already clear it. Overall lines 91.3 % → **92.0 %**.
4. ~~Re-measure; record the real per-package baseline.~~ **DONE 2026-06-25** — see baseline below.
5. **Lint-scope gap (closed 2026-09-25, PR #83)** — until then the gate ran `tsc --noEmit` only in `@mas/memory`, `@mas/skills` and `@mas/web`; `@mas/core`, `@mas/agents`, `@mas/db` and `@mas/worker` had no `lint` script, so `pnpm lint` (and CI) never type-checked them and 28 test-file type errors (25 core, 3 agents) sat on `main` unmeasured. All four now declare `"lint": "tsc --noEmit"`.
