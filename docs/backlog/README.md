...
- [`perms-category-matcher-brittleness.md`](perms-category-matcher-brittleness.md) — **câblage §5 (2026-09-13)** : `config/permissions.json#categories` était vide, le portique n'avait rien à comparer ; désormais rempli + verrouillé par 2 tests. Reste que le match est en **sous-chaîne** : `path-escape` et `outbound-network` ne seront étanches qu'avec le seam runtime de `allowed-hosts-runtime-gate.md` (même point de branchement). **Sécurité, filet posé, étanchéité à finir.**
