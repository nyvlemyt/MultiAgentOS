...
- [`per-provider-subscription-awareness.md`](per-provider-subscription-awareness.md) — **l'app doit toujours connaître le plan/abonnement de chaque IA** (Claude Max 100 €, OpenAI PAYG, Gemini free…). Gap : `config/model-routing.json` n'a pas de champ `plan`/quota. Lié au budget-gate (PR #33) + §11 billing. **Exigence user permanente (2026-06-21).**
...
- [`perms-category-matcher-brittleness.md`](perms-category-matcher-brittleness.md) — **câblage §5 (2026-09-13 → 15)** : `categories` était vide (rempli + verrouillé par 2 tests le 13), puis la moitié **chemin** a reçu son contrôle runtime le 15 (`path-guard.ts` : `..`, chemins absolus, évasion par symlink, `.git/` → tâche `blocking` + pause humaine, zéro appel critique). La moitié hôte l'avait déjà (net-guard). Reste : les écritures directes du moteur (hook `canUseTool` absent) et le net-guard sur le chemin dispatch. **Sécurité — deux moitiés couvertes au niveau diff/fetch, frontière outil à faire.**
...
- [`knowledge-os-reste-a-faire.md`](knowledge-os-reste-a-faire.md) — **carte de reprise (2026-07-10)** : les 4 actions restantes du chantier Knowledge OS après les briques d'ingestion (#57→#62 vérifiées mergées) — fichier `.url` dashboard + règle, capture de masse S1→S7 (machine macOS), Brique 5 cockpit, merge `brique-1`→`main`. **À ouvrir en premier par la prochaine session Knowledge OS.**
- [`visual-effects-arsenal-intake-queue.md`](visual-effects-arsenal-intake-queue.md) — **file d'intake effets visuels (2026-07-10)** : 7 candidats en pyramide (react-three-fiber fondation → shadergradient/liquid-logo/liquid-glass → skiper-ui/vengenceui/animmasterlib), URLs exactes + grille d'audit commune (§11 freemium, Sanitize, doublons, dépendance-vs-arsenal) + alerte sécurité 0xLoucash (1 lien injection rejeté, 1 guide en quarantaine). Version canonique du poste Windows (`83fe731`), restaurée après une reconstruction temporaire côté macOS.
...
- [`classifieur-faux-positifs-cours.md`](classifieur-faux-positifs-cours.md) — **faux positifs du classifieur (2026-09-04, PR #77)** : les 51 candidats « classés » sont 51 documents de cours mal rangés — 7 TD DevOps en `blockers` (« don't stay blocked » dans la consigne), 1 « Score Report » en `evals`, 43 supports en `learnings` (« Deep **Learning** » = le mot du domaine, pas du registre). La table mots-clés de `classifyByRulesOnly` a été calibrée sur des candidats de *mission* puis réutilisée sur de la prose de *cours* : le signal manquant (`trust='untrusted'` + provenance convoyeur) est déjà en base et n'est pas lu. **Crédibilité des registres, mécanique saine. Rollback fourni.**
- [`sonar-path-guard-test-duplication.md`](sonar-path-guard-test-duplication.md) — **résidu de duplication Sonar de la PR #82 (2,97 %)** : les deux blocs signalés sont dans `packages/core/src/path-guard.test.ts` (six `it()` d'une ligne → `it.each`), pas dans les suites dispatch. Correctif ~10 lignes ; vérifier les blocs par l'API avant d'attribuer un résidu.
