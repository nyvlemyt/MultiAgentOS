...
class NormalisationTests(unittest.TestCase):
    ...
    def test_le_decoupage_profond_rend_en_plus_la_commande_dun_shell_imbrique(self) -> None:
        """`bash -c "rm x"` arrive en un seul token : sans redecoupage, aucun verrou ne voit `rm`.
        La profondeur 1 est le defaut et reproduit exactement le comportement historique."""
        plat = _lib.decouper_commande('bash -c "rm C:/dev/maos/x.md"')
        self.assertEqual([["bash", "-c", "rm C:/dev/maos/x.md"]], plat)
        profond = _lib.decouper_commande('bash -c "rm C:/dev/maos/x.md"', profondeur=2)
        self.assertIn(["bash", "-c", "rm C:/dev/maos/x.md"], profond)
        self.assertIn(["rm", "C:/dev/maos/x.md"], profond)

    def test_le_decoupage_profond_prend_tous_les_tokens_qui_suivent_loption(self) -> None:
        """`cmd /c del x` a deja ses arguments en tokens separes : prendre le seul token suivant
        laisserait le chemin orphelin."""
        profond = _lib.decouper_commande("cmd /c del C:/dev/maos/x.md", profondeur=2)
        self.assertIn(["del", "C:/dev/maos/x.md"], profond)
        pwsh = _lib.decouper_commande('pwsh -NoProfile -Command "Remove-Item C:/dev/maos/x.md"', profondeur=2)
        self.assertIn(["Remove-Item", "C:/dev/maos/x.md"], pwsh)

    def test_le_decoupage_profond_sarrete_a_sa_limite(self) -> None:
        profond = _lib.decouper_commande('bash -c "bash -c \\"rm C:/dev/maos/x.md\\""', profondeur=2)
        self.assertTrue(any("bash" in sc for sc in profond), profond)

    def test_extraction_des_chemins_dune_commande(self) -> None:
        chemins = _lib.extraire_chemins_commande(
            'head -5 "C:/dev/Eve/Providers/ISS_ESG/export.xlsx" && cat data/api.py'
        )
...
class GardePerimetreTests(unittest.TestCase):
    ...
    def test_un_gestionnaire_de_paquets_qui_installe_nest_pas_une_copie_de_fichier(self) -> None:
        ...
        self.assertIsNone(self.decision("Bash", {"command": "npm install --prefix C:/dev/csdr_codex"}))
...
class DoctorControlesDuDispositifTests(unittest.TestCase):
    ...
    def test_les_deux_controles_sont_branches_dans_verifications(self) -> None:
        ...
        lignes = ok + alertes
        self.assertTrue(any("fiches d'agents" in l for l in lignes), lignes)
        self.assertTrue(any("scripts declares" in l for l in lignes), lignes)
