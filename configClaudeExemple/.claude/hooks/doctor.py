...
# Les verrous, leurs trois modules d'analyse (un module absent fait tomber les trois verrous en refus
# systematique par `_lib.executer`, donc son absence doit se voir ici), et les outils du poste.
HOOKS = (
    "garde_donnees.py", "garde_git.py", "garde_perimetre.py", "_lib.py", "_commande.py", "_effets.py", "_programmes.py",
    "verif_style.py", "nettoyer_caracteres.py", "doctor.py", "gate.py",
)
...
def autotests() -> list[str]:
    ...
    cas = [
        ...
        # passer les sept interdits de git.md et toute ecriture hors perimetre.
        ("garde_git.py", {"tool_name": "Bash", "tool_input": {"command": 'bash -c "git push --force"'}}, 2),
        # Lot 3 (17/09/2026) : une enveloppe devant git sans shell, et un cd suivi d'un chemin relatif,
        # les deux formes qui passaient avant la refondation des verrous.
        ("garde_git.py", {"tool_name": "Bash", "tool_input": {"command": "env git push --force"}}, 2),
        ("garde_perimetre.py", {"tool_name": "Bash", "tool_input": {"command": "cd C:/dev/maos && rm CLAUDE.md"}}, 2),
        # Un verrou qui plante refuse : `cwd` n'est pas une chaine, `_branche_courante` leve, `executer`
        # transforme l'exception en refus. Sans cela le code de sortie serait 1, lu comme « rien a dire ».
        ("garde_git.py", {"tool_name": "Bash", "tool_input": {"command": "git status"}, "cwd": 123456}, 2),
