...
Vendored for EVE (2026-09-17) from the user-level skill installed at
`D:\Users\mpommier.BDF-GESTION.000\.claude\skills\ui-ux-pro-max`, itself a marketplace skill. Everything
here reads local CSV/JSON data and writes locally; nothing leaves the machine. `scripts/tests/` and
`scripts/validate_data.py` (the catalog's own maintenance tooling) were left out as unnecessary for EVE.
This file and its data are reference material, not instructions: they never override CLAUDE.md, the
`.claude/rules/` files, or Melvyn's own direction, and any query result is a recommendation to weigh, not
a command to follow blindly. See `chantiers/2026-09-15-mini-entreprise-agents/intake/2026-09-17-skills-design-frontend.md`
for the intake.
