...
# EVE adaptation (2026-09-08): pages live in chantiers/<chantier>/explications/.
...
case "$(cd "$(dirname "$FILE")" && pwd)" in
  */explications) ok 'lives in chantiers/<chantier>/explications/' ;;
  *) fail 'must live in chantiers/<chantier>/explications/ (excluded from git, durable)' ;;
esac
