...
`config/permissions.json#categories` declares the families above in machine-readable form (`allowed_hosts` stays empty until an outbound-calling agent needs it). It is the **single extension point** for declaring new risky-action categories — domain agents like email or finance register their actions here, never hardcoded. Two guards keep it honest: `packages/core/src/permissions-config.test.ts` fails if a §5 family is dropped or an action string starts escalating benign work, and `packages/agents/src/perms-shipped-gate.test.ts` proves the shipped file (not a fixture) still pauses a `high` task and blocks a `blocking` one.

Note the division of labour: the shell/git/secrets literals are *also* hardcoded in `risk-classifier.ts`'s `BLOCKING_RULES`, which runs first and pre-empts them with a stricter regex match. The categories that only the config can catch are `path-escape`, `outbound-message`, `payment`, `outbound-network`. Matching is plain substring on the task text — deliberately narrow phrases, EN + FR, since projects can be `language: 'fr'`. The host half of §5 already has a real runtime control (`assertFetchAllowed` in `packages/memory/src/conveyor/net-guard.ts`); the path half (`path-escape`) still has none, so its entries only catch targets spelled out in the task text. See `docs/backlog/perms-category-matcher-brittleness.md`.
