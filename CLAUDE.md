...
`config/permissions.json#categories` declares the families above in machine-readable form (`allowed_hosts` carries the conveyor's fetch allowlist, enforced by `net-guard.ts`). It is the **single extension point** for declaring new risky-action categories — domain agents like email or finance register their actions here, never hardcoded. Two guards keep it honest: `packages/core/src/permissions-config.test.ts` fails if a §5 family is dropped or an action string starts escalating benign work, and `packages/agents/src/perms-shipped-gate.test.ts` proves the shipped file (not a fixture) still pauses a `high` task and blocks a `blocking` one.

Note the division of labour: the shell/git/secrets literals are *also* hardcoded in `risk-classifier.ts`'s `BLOCKING_RULES`, which runs first and pre-empts them with a stricter regex match. The categories that only the config can catch are `path-escape`, `outbound-message`, `payment`, `outbound-network`. Matching is plain substring on the task text — deliberately narrow phrases, EN + FR, since projects can be `language: 'fr'`. Both halves of §5 now have a real runtime control — host: `assertFetchAllowed` (`packages/memory/src/conveyor/net-guard.ts`, every conveyor fetch); path: `assertDiffWithinProject` (`packages/core/src/path-guard.ts`, every Tier-B diff before any critic — `..` traversal, absolute paths, symlink escapes via injected `realpath`, `.git/`; a hit escalates the task to `blocking` and pauses it through `pauseForPathEscape`). Still text-only: writes the Claude Code engine makes directly through its own tools (no `canUseTool` hook yet) — see `docs/backlog/perms-category-matcher-brittleness.md`.
...
- **Coding standards arsenal**: per-language standards live in `docs/rules/<lang>/<concern>.md` (ECC harvest). For our active stack (TS/React/Next/web) these are **active rules**, not just reference — the non-obvious ones:
  ...
  - Binary review thresholds: fn < 50 lines · file < 800 · nesting ≤ 4 · coverage. **`file < 800` is machine-enforced, not advisory** (intake 2026-07-31 God-file): `.claude/hooks/limit-file-size.sh` denies the write (pedagogic floor, source files only), `scripts/check-max-lines.sh` in `pnpm lint` denies the merge (uncrossable floor). Exceptions are listed in both scripts and each owns a backlog card. Coverage is now an **enforced blocking gate** — the 6th verification check (`pnpm test:coverage`, v8 provider). Global floor: **lines/statements ≥ 92 % · functions ≥ 92 % · branches ≥ 84 %** (set in `vitest.config.ts` `test.coverage.thresholds`, ratcheted just below the 2026-09-04 measured baseline of 95.6 % lines / 86.3 % branches / 95.3 % functions). Dev-only CLI shims (`*-cli.ts`, `build-library-index.ts`, `reindex.ts`) + drizzle `schema.ts`/`seed.ts` are excluded so the gate measures real logic, not 0 % entrypoints. Scope = `packages/*/src` (apps not yet gated). See `docs/backlog/test-coverage-measurement-gap.md`.
  - **No attribution footer in commits** — never `Co-Authored-By`, never an email address (owner rule). ECC's "attribution disabled" git rule matches ours, so adopt it rather than reject it. A PR body may credit a **co-editor**; never a co-author.
- **Verification = 6 checks, not 5.** A phase/PR is done only when `pnpm -r test` · `pnpm lint` · `pnpm build` · `pnpm --filter @mas/web smoke` · `pnpm test:coverage` **and** SonarCloud are all clean. Sonar clean means `scripts/sonar-pr-issues.sh <pr>` exits 0 — **zero open issues and zero to-review hotspots**, not merely a green gate (the gate ignores MINOR/MAJOR smells). After `git push`, poll until the analysis of your HEAD sha lands, run the script, and fix everything it lists. Recurring rules + canonical fixes: `docs/knowledge/sonar-recurring-rules.md` — read it before writing UI/test code to avoid the round-trips.
...
7. **Dashboard visuel de suivi (systématique).** Pour tout sujet structurant — plan, architecture, schéma de données, avancement multi-étapes — produire et **maintenir** un dashboard visuel (artifact HTML : diagrammes, timelines, statuts, points à valider) plutôt qu'un long texte. Une même mission = une même page, mise à jour au fil de l'avancement (jamais une nouvelle page par étape). Le texte accompagne le visuel, pas l'inverse. Méthode complète : `docs/workflows/dashboard-visuel-de-suivi.md`.
